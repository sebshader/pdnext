    ...
    if (vis)
    {
        ...
        if (x->gl_xtick.k_lperb)
        {
                    ...
                    f += x->gl_xtick.k_inc)
            {
                ...
                sys_vgui(".x%lx.c create line %d %d %d %d -width %d -fill %s -tags [list %s graph]\n",
                    glist_getcanvas(x->gl_owner),
                    (int)glist_xtopixels(x, f), (int)upix,
                    (int)glist_xtopixels(x, f), (int)upix - tickpix, glist_getzoom(x), selected, tag);
                sys_vgui(".x%lx.c create line %d %d %d %d -width %d -fill %s -tags [list %s graph]\n",
                    glist_getcanvas(x->gl_owner),
                    (int)glist_xtopixels(x, f), (int)lpix,
                    (int)glist_xtopixels(x, f), (int)lpix + tickpix, glist_getzoom(x), selected, tag);
            }
        ...
        if (x->gl_ytick.k_lperb)
        {
                    ...
                    i++, f += x->gl_ytick.k_inc)
            {
                ...
                sys_vgui(".x%lx.c create line %d %d %d %d -width %d -fill %s -tags [list %s graph]\n",
                    ...
                    x1 + tickpix, (int)glist_ytopixels(x, f), glist_getzoom(x), selected, tag);
                sys_vgui(".x%lx.c create line %d %d %d %d -width %d -fill %s -tags [list %s graph]\n",
                    glist_getcanvas(x->gl_owner),
                    x2, (int)glist_ytopixels(x, f),
                    x2 - tickpix, (int)glist_ytopixels(x, f), glist_getzoom(x), selected, tag);
            }
...
static void graph_select(t_gobj *z, t_glist *glist, int state)
{
    ...
    else
    {
        ...
        sys_vgui(".x%lx.c itemconfigure graph%lx -fill %s\n",
            glist_getcanvas(glist), z, (state? "$select_color" : "$graph_outline"));
    }
}
