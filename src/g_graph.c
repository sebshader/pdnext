...
static void graph_vis(t_gobj *gr, t_glist *parent_glist, int vis)
{
 ...
 %d %d %d %d %d %d %d %d %d %d -tags [list %s graph] -fill #c0c0c0\n",
    ...
    if (vis)
    {
        ...
        sys_vgui(".x%lx.c create line\
            %d %d %d %d %d %d %d %d %d %d -fill %s -tags [list %s graph]\n",
            glist_getcanvas(x->gl_owner),
            x1, y1, x1, y2, x2, y2, x2, y1, x1, y1, selected, tag);
        ...
        if (x->gl_xtick.k_lperb)
        {
                    ...
                    f += x->gl_xtick.k_inc)
            {
                ...
                sys_vgui(".x%lx.c create line %d %d %d %d -fill %s -tags [list %s graph]\n",
                    glist_getcanvas(x->gl_owner),
                    (int)glist_xtopixels(x, f), (int)upix,
                    (int)glist_xtopixels(x, f), (int)upix - tickpix, selected,
                    tag);
                sys_vgui(".x%lx.c create line %d %d %d %d -fill %s -tags [list %s graph]\n",
                    glist_getcanvas(x->gl_owner),
                    (int)glist_xtopixels(x, f), (int)lpix,
                    (int)glist_xtopixels(x, f), (int)lpix + tickpix, selected,
                    tag);
            }
        ...
        if (x->gl_ytick.k_lperb)
        {
                    ...
                    i++, f += x->gl_ytick.k_inc)
            {
                ...
                sys_vgui(".x%lx.c create line %d %d %d %d -fill %s -tags [list %s graph]\n",
                    ...
                    x1 + tickpix, (int)glist_ytopixels(x, f), selected, tag);
                sys_vgui(".x%lx.c create line %d %d %d %d -fill %s -tags [list %s graph]\n",
                    glist_getcanvas(x->gl_owner),
                    x2, (int)glist_ytopixels(x, f),
                    x2 - tickpix, (int)glist_ytopixels(x, f), selected, tag);
            }
 ...
 %d %d -text {%s} -font {{%s} -%d %s} -anchor %s -fill %s -tags [list %s label graph]\n",
                glist_getcanvas(x),
                (int)glist_xtopixels(x, atof(x->gl_xlabel[i]->s_name)),
                (int)glist_ytopixels(x, x->gl_xlabely),
                x->gl_xlabel[i]->s_name, sys_font,
                     glist_getfont(x), sys_fontweight, xlabelanchor, selected, 
                     tag);
 ...
 %d %d -text {%s} -font {{%s} -%d %s} -anchor %s -fill %s -tags [list %s label graph]\n",
                ...
                glist_getfont(x), sys_fontweight, ylabelanchor, selected, tag);
